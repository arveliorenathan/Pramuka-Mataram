...
export async function PATCH(
  ...
  if (!session || session.user.role !== "USER_GUSDEP") {
    return NextResponse.json(
      { message: "Unauthorized: Only 'Gugus Depan' users edit add member" },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (anggota.gusdepKode !== user.kode_gusdep) {
      return NextResponse.json(
        { message: "You can only edit members from your own Gugus Depan" },
        { status: 403 }
      );
    }
    ...
    if (body.nta && body.nta !== anggota.nta) {
      const existingNTA = await prisma.anggota.findUnique({
        where: { nta: body.nta },
      });
      if (existingNTA) {
        return NextResponse.json(
    ...
    const updatedAnggota = await prisma.anggota.update({
      ...
      data: {
        ...
        ...(body.tahun_gabung &&
          !isNaN(parseInt(body.tahun_gabung)) && {
            tahun_gabung: parseInt(body.tahun_gabung),
          }),
      },
    });

    return NextResponse.json(
      { message: "Member successfully updated", data: updatedAnggota },
      { status: 200 }
    );
  } catch (error) {
...
export async function DELETE(
  ...
  if (!session || session.user.role !== "USER_GUSDEP") {
    return NextResponse.json(
      { message: "Unauthorized: Only 'Gugus Depan' users can delete members" },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (anggota.gusdepKode !== user.kode_gusdep) {
      return NextResponse.json(
        { message: "You can only delete members from your own Gugus Depan" },
        { status: 403 }
      );
    }
