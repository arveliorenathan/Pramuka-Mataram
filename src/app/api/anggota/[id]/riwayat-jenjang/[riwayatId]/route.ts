...
export async function DELETE(
  ...
  if (!session || session.user.role !== "USER_GUSDEP") {
    return NextResponse.json(
      {
        message:
          "Unauthorized: Only 'Gugus Depan' users can delete riwayat jenjang",
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (anggota.gusdepKode !== user.kode_gusdep) {
      return NextResponse.json(
        { message: "You do not have the right to delete this data" },
        { status: 403 }
      );
    }
    ...
    if (!riwayat || riwayat.anggotaId !== anggotaId) {
      return NextResponse.json(
        { message: "Riwayat jenjang not found or invalid" },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { message: "Riwayat jenjang successfully deleted" },
      { status: 200 }
    );
  } catch (error) {
