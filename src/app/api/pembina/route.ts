...
export async function POST(req: NextRequest) {
  ...
  if (!session || session.user.role !== "USER_GUSDEP") {
    return NextResponse.json(
      { message: "Unauthorized: Only 'Gugus Depan' users can add mentor" },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    const nama_pbn = body.nama_pbn?.trim(),
      nta = body.nta?.trim(),
    ...
    return NextResponse.json(
      { message: "Mentor successfully added", pembina },
      { status: 201 }
    );
  } catch (error) {
...
export async function GET(req: NextRequest) {
  ...
  if (!session || session.user.role === "USER_SUPERADMIN") {
    return NextResponse.json(
      {
        message:
          "Unauthorized: Only 'Kwarcab/Kwaran/Gusdep' users can retrieve mentors",
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (session.user.role === "USER_GUSDEP") {
      ...
      if (!session.user.kode_kwaran) {
        return NextResponse.json(
          { message: "Kode kwaran tidak ditemukan di session" },
          { status: 400 }
        );
      }
      ...
      if (!session.user.kode_kwarcab) {
        return NextResponse.json(
          { message: "Kode kwarcab tidak ditemukan di session" },
          { status: 400 }
        );
      }
    ...
    console.log(
      `Role: ${session.user.role} | Total mentor found: ${result.length}`
    );
