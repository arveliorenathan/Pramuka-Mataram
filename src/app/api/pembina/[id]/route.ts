...
export async function PATCH(
  ...
  if (!session || session.user.role !== "USER_GUSDEP") {
    return NextResponse.json(
      { message: "Unauthorized: Only 'Gugus Depan' users edit add mentor" },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (pembina.gusdepKode !== user.kode_gusdep) {
      return NextResponse.json(
        { message: "You can only edit mentors from your own Gugus Depan" },
        { status: 403 }
      );
    }
    ...
    if (body.nta && body.nta !== pembina.nta) {
      const existingNTA = await prisma.pembina.findUnique({
        where: { nta: body.nta },
      });
      if (existingNTA) {
        return NextResponse.json(
    ...
    if (jenjang_pbn && !isValidEnum("JenjangPembina", jenjang_pbn)) {
      return NextResponse.json(
        { message: "Invalid jenjang pembina" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: "Mentor successfully updated", data: updatedPembina },
      { status: 200 }
    );
  } catch (error) {
...
export async function DELETE(
  ...
  if (!session || session.user.role !== "USER_GUSDEP") {
    return NextResponse.json(
      { message: "Unauthorized: Only 'Gugus Depan' users can delete mentors" },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (pembina.gusdepKode !== user.kode_gusdep) {
      return NextResponse.json(
        { message: "You can only delete mentors from your own Gugus Depan" },
        { status: 403 }
      );
    }
